...
function ktz_author_by() {
	printf( __( '<span class="entry-author vcard">By <a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>', ktz_theme_textdomain ),
...
if ( !function_exists('ktz_author_by_attachment') ) :
function ktz_author_by_attachment() {
	printf( __( '<span class="vcard">By <a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>', ktz_theme_textdomain ),
		esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
		esc_attr( sprintf( __( 'View all posts by %s', ktz_theme_textdomain ), get_the_author() ) ),
		get_the_author()
	);
...
function ktz_content() {
		if ( ot_get_option('ktz_active_automore') == 'yes' && !(is_single()) ) // Set active/deactive via admin
			...
			{ 
				the_content( __( '...', ktz_theme_textdomain ) ); 
			}
}
endif;

/*
* Add content on hook system
* Call only for wallpaper themes
*/
// first paragraph or before image
if ( !function_exists('ktz_content_single_firstp') ) :
function ktz_content_single_firstp() {
	global $post;
	if ( ot_get_option('ktz_active_autocontent') == 'yes' ) {
	$category = get_the_category(); 
    $content_format = ot_get_option('ktz_wall_autocontent_firstp');
...
// second paragraph or after image
if ( !function_exists('ktz_content_single') ) :
function ktz_content_single() {
	global $post;
	if ( ot_get_option('ktz_active_autocontent') == 'yes' ) {
	$category = get_the_category(); 
    $content_format = ot_get_option('ktz_wall_autocontent');
    $ktz_autocontent = str_replace( '%ktzblogname%', get_bloginfo('name') , $content_format );
	$ktz_autocontent = str_replace( '%ktzhomeurl%', get_home_url() , $content_format );
    $ktz_autocontent = str_replace( '%ktzblogdescription%', get_bloginfo('description') , $ktz_autocontent );
    $ktz_autocontent = str_replace( '%ktztitle%', get_the_title() , $ktz_autocontent );
...
*/
// first paragraph or before image
if ( !function_exists('ktz_content_single_threep_attachement') ) :
