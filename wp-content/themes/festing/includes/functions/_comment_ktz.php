...
function ktz_com_fields_textarea($fields_com) {
...
function ktz_comments($comment, $args, $depth) {
		...
		<div class="commentwrapper clearfix <?php $author_id = get_the_author_meta('ID'); if($author_id == $comment->user_id) $author_flag = 'true';?>" id="comment-<?php comment_ID(); ?>">
			...
			<span class="fontawesome ktzfo-caret-left"></span>
                ...
                <p><span class="comment_author_link"><?php comment_author_link(); ?></span><span class="comment-date"><?php printf( __( '%1$s',ktz_theme_textdomain ), human_time_diff( get_comment_time('U'), current_time('timestamp') ) . ' ago',  get_comment_time() );?></span></p>
...
* Add add_action( 'do_ktz_comment_template', 'ktz_comments_template' ); in init.php 
*/
...
* Add add_action( 'do_ktz_comment_facebook', 'ktz_comments_facebook' ); in init.php 
*/
if ( !function_exists('ktz_comments_facebook') ) {
function ktz_comments_facebook() { 
	if ( ot_get_option('ktz_facebook_com_activated') == 'yes' ) : 
    echo '<div class="fb-comments" data-href="' . get_permalink() . '" data-width="539"></div>';  
	endif;
	} 	
}
