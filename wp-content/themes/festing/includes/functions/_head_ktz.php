...
* add_filter( 'language_attributes', 'add_opengraph_doctype' ); in init.php
*/
...
function ktz_headelement() {
	...
	*/
	...
	$ktz_metadata_arg[] = '<link rel="profile" href="http://gmpg.org/xfn/11" />';
...
* add_filter( 'wp_title', 'ktz_dynamic_title', 10, 2 ); in init.php
*/
...
* add_action( 'do_ktz_topsearch', 'ktz_topsearch' ); in init.php
*/
if( !function_exists('ktz_topsearch')) {
	function ktz_topsearch() {
	$search_text = empty($_GET['s']) ? "Search" : get_search_query();
	echo '<div id="search">';
    echo '<form method="get" id="searchform" action="' . esc_url( home_url( '' ) ) . '"><div class="form-inline">'; 
    echo '<input type="text" name="s" id="s" class="form-control btn-box" placeholder="Search and enter" />';
	echo '</div>';
    echo '</form>';
	echo '</div>';
	}
