...
/**
 * ktzagcplugin Sanitization Helper Class
 *
 ...
 */
class Ktzagcplugin_Sanitization_Helper {
	...
	private function apply_type_filter( $input, $tab, $key ) {
		...
		return apply_filters( 'ktzagcplugin_settings_sanitize_' . $type, $input[$key], $key );
	}

	private function apply_general_filter( $input, $key ) {

		return apply_filters( 'ktzagcplugin_settings_sanitize', $input[$key], $key );
	}

	// Key specific on change hook
	private function do_settings_on_key_change_hook( $key, $new_value ) {

		$old_plugin_settings = get_option( 'ktzagcplugin_settings' );
		...
		if ( isset($old_plugin_settings[$key]) && $old_plugin_settings[$key] !== $new_value ) {
			...
			do_action( 'ktzagcplugin_settings_on_change_' . $key, $new_value, $old_plugin_settings[$key] );
	...
	private function do_settings_on_change_hook( $new_values, $tab ) {
		...
		if ( $changed ) {
			...
			do_action( 'ktzagcplugin_settings_on_change_' . $tab, $new_values, $old_plugin_settings );
